"""
Download videos listed in an Excel file (columns: Name, URL).
Uses yt-dlp for page URLs (e.g. xHamster). Install: pip install -r requirements.txt
"""
...
def _build_ydl_opts_for_mode(mode: str, outtmpl: str, base: dict) -> dict:
...
def main() -> int:
    ...
    parser = argparse.ArgumentParser(description="Download videos from Excel Name + URL columns.")
    parser.add_argument(
        "excel",
        nargs="?",
        default="List.xlsx",
        help="Path to .xlsx file (default: List.xlsx)",
    )
    parser.add_argument(
        "-o",
        "--output-dir",
    ...
    if not excel_path.is_file():
        print(f"Excel not found: {excel_path.resolve()}", file=sys.stderr)
        return 1
    ...
    for idx, row in df.iterrows():
        ...
        ydl_opts = _build_ydl_opts_for_mode(mode, outtmpl, base_opts)
        if args.impersonate:
            ydl_opts["impersonate"] = ImpersonateTarget.from_str(args.impersonate.lower())
